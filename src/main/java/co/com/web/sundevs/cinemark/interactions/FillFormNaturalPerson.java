...
public class FillFormNaturalPerson implements Interaction {
    ...
    public <T extends Actor> void performAs(T actor) {
        actor.attemptsTo(
                ...
                Click.on(BTN_OPTION_FROM_LIST_1.of(user.getDocumentType())),
                Enter.theValue(user.getName()).into(LBL_NAME),
                Enter.theValue(user.getLastName()).into(LBL_LASTNAME),
                Click.on(LST_GENDER),
                Click.on(BTN_OPTION_FROM_LIST_1.of(user.getGender())),
                ...
                Click.on(LST_CITY),Click.on(BTN_OPTION_FROM_LIST_3.of(user.getCity())),
                Click.on(LST_PREFERRED_THEATER),
                Click.on(BTN_OPTION_FROM_LIST_3.of(user.getPreferredTheater())),
